// Written by Aaron Längert, modified by JuniorBecari10
...
var ripple = (function () {
  ...
  function rippleStart(e) {
    let rippleContainer = getRippleContainer(e.target);
    ...
    if (
      ...
      let fullCoverRadius = Math.max(
        ...
        Math.sqrt(
          Math.pow(offsetX, 2) + Math.pow(e.target.clientHeight - offsetY, 2)
        ),
        Math.sqrt(
          Math.pow(offsetY, 2) + Math.pow(e.target.clientWidth - offsetX, 2)
        )
      );

      let expandTime = e.target.getAttribute("ripple-press-expand-time") || 3;

      rippleContainer.style.transition =
        "transform " + expandTime + "s ease-out, box-shadow 0.1s linear";
  ...
  function rippleEnd(e) {
    ...
    if (rippleContainer.getAttribute("animating") == "1") {
      ...
      e.target.dispatchEvent(new CustomEvent("ripple-button-click", { target: e.target }));
  ...
  function getRippleContainer(el) {
    ...
    for (let ii = 0; ii < childs.length; ii += 1) {
      try {
  ...
  var ripple = {
    registerRipples: function () {
      let rippleButtons = document.getElementsByClassName("ripple");

      for (let i = 0; i < rippleButtons.length; i += 1) {
    ...
    ripple: function (el) {
      ...
      e = { target: el, offsetX: rect.width / 2, offsetY: rect.height / 2 };
